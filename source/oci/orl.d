...
/*
  ...
  PUBLIC FUNCTIONS
    ...
    OCIColl_ - OCI Collection generic functions. These functions can be
                used to manipulate both variable-length array (varray) and
                nested table.
      OCICollSize  - OCIColl_ return current SIZe of the given collection
                (in number of elements)
      OCICollMax  - OCIColl_ return the MAXimum number of elements in the
                collection (i.e. upper-bound)
      OCICollGetElem  - OCIColl_ GET pointer to the element at the given
                position
      OCICollAssignElem  - OCIColl_ assign to element at given index
      OCICollAssign  - OCIColl_ ASsiGn collection; perform deep-copy of source
                collection to target collection
      OCICollAppend  - OCIColl_ aPPend the given element to the end of the
                collection
      OCICollTrim  - OCIColl_ trim (delete) the given number of elements
                from the end of the collection
      OCICollIsLocator - OCIColl_ indicates whether a collection is locator
                         based or not.
      OCIIterCreate  - OCIColl_ Create an ITerator to scan the collection
                elements
      OCIIterDelete  - OCIColl_ Delete ITerator
      OCIIterInit  - OCIColl_ Initialize ITerator to scan the given collection
      OCIIterGetCurrent - OCIColl_ Iterator based, get CURrent
                collection element
      OCIIterNext - OCIColl_ Iterator based, get NeXT collection element
      OCIIterPrev - OCIColl_ Iterator based, get PReVious collection element

      OCITable - OCI functions to manipulate nested Table. The OCIColl_*() and
    ...
    OCIRef_ - OCI functions to manipulate object Reference
      OCIRefClear  - OCIRef_ CLeaR or nullify a ref
      OCIRefAssign  - OCIRef_ ASsiGn a ref to another
      OCIRefIsEqual  - OCIRef_ compare two refs for EQUality
      OCIRefIsNull  - OCIRef_ test if a ref is NULl
      OCIRefFromHex  - OCIRef_ convert a Hexadecimal string TO a Ref
      OCIRefToHex   - OCIRef_ convert a ref to a Hexadecimal string
      OCIRefHexSize  - OCIRef_ get size of buffer in bytes to store hexadecimal
                string
...
enum OCI_NUMBER_UNSIGNED = 0; /* Unsigned type -- ubX */
enum OCI_NUMBER_SIGNED = 2; /* Signed type -- sbX */
...
sword OCINumberIsZero (OCIError* err, const(OCINumber)* number, int* result);
/*
...
sword OCINumberIsInt (OCIError* err, const(OCINumber)* number, int* result);
/*
...
enum OCI_DATE_INVALID_DAY = 0x1; /* Bad DAy */
enum OCI_DATE_DAY_BELOW_VALID = 0x2; /* Bad DAy Low/high bit (1=low)*/
enum OCI_DATE_INVALID_MONTH = 0x4; /* Bad MOnth */
enum OCI_DATE_MONTH_BELOW_VALID = 0x8; /* Bad MOnth Low/high bit (1=low)*/
enum OCI_DATE_INVALID_YEAR = 0x10; /* Bad YeaR */
enum OCI_DATE_YEAR_BELOW_VALID = 0x20; /* Bad YeaR Low/high bit (1=low)*/
enum OCI_DATE_INVALID_HOUR = 0x40; /* Bad HouR */
enum OCI_DATE_HOUR_BELOW_VALID = 0x80; /* Bad HouR Low/high bit (1=low)*/
enum OCI_DATE_INVALID_MINUTE = 0x100; /* Bad MiNute */
enum OCI_DATE_MINUTE_BELOW_VALID = 0x200;
/* Bad MiNute Low/high bit (1=low)*/
enum OCI_DATE_INVALID_SECOND = 0x400; /* Bad SeCond */
enum OCI_DATE_SECOND_BELOW_VALID = 0x800;
/* bad second Low/high bit (1=low)*/
enum OCI_DATE_DAY_MISSING_FROM_1582 = 0x1000;
/* Day is one of those "missing" from 1582 */
enum OCI_DATE_YEAR_ZERO = 0x2000; /* Year may not equal zero */
enum OCI_DATE_INVALID_FORMAT = 0x8000; /* Bad date format input */
...
/*
 * See the definition of OCIRef_ in oro.h.
 *
 * For binding variables of type OCIRef_* in OCI calls (OCIBindByName(),
 * OCIBindByPos() and OCIDefineByPos()) use the code SQLT_REF.
 *
 */

/*---------------------------- OCIRefClear ----------------------------------*/
void OCIRefClear (OCIEnv* env, OCIRef_* ref_);
/*
   NAME: OCIRefClear - OCIRef_ CLeaR or nullify a ref
   PARAMETERS:
        env (IN)     - pointer to OCI environment handle
...
sword OCIRefAssign (
    OCIEnv* env,
    OCIError* err,
    const(OCIRef_)* source,
    OCIRef_** target);
/*
   NAME: OCIRefAssign - OCIRef_ CoPY a ref to another
   PARAMETERS:
        env (IN/OUT) - OCI environment handle initialized in object mode.
        err (IN/OUT) - error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
...
/*-------------------------- OCIRefIsEqual ----------------------------------*/
int OCIRefIsEqual (OCIEnv* env, const(OCIRef_)* x, const(OCIRef_)* y);
/*
   NAME: OCIRefIsEqual - OCIRef_ compare two refs for EQUality
   PARAMETERS:
        env (IN) - pointer to OCI environment handle
        x (IN)   - ref to compare
        y (IN)   - ref to compare
...
/*--------------------------- OCIRefIsNull ----------------------------------*/
int OCIRefIsNull (OCIEnv* env, const(OCIRef_)* ref_);
/*
   NAME: OCIRefIsNull - OCIRef_ test if a ref is NULl
   PARAMETERS:
        env (IN) - pointer to OCI environment handle
...
/*-------------------------- OCIRefHexSize ----------------------------------*/
ub4 OCIRefHexSize (OCIEnv* env, const(OCIRef_)* ref_);
/*
   NAME: OCIRefHexSize - OCIRef_ Hexadecimal buffer SiZe in bytes
   PARAMETERS:
        env (IN) - pointer to OCI environment handle
...
/*
   NAME:
        OCIRefToHex - OCIRef_ convert ref to a Hexadecimal string
   PARAMETERS:
        env (IN/OUT) - OCI environment handle initialized in object mode
        err (IN/OUT) - error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
...
/*
   The generic collection is represented by the type 'OCIColl_'. The following
   operations OCIColl_*() are provided on a generic collection:
   ...
   are sub-types of generic collection. This means that the OCIColl_*()
   ...
   Besides OCIColl_*() functions no additional functions are provided for
   manipulating varrays. The OCIColl_*() functions are a complete set of
   functions to manipulate varrays.

   Besides OCIColl_*() functions, the following functions OCITable*() can be
   ...
   For binding variables of type OCIColl_* or OCITable* in OCI calls
   (OCIBindByName(), OCIBindByPos() and OCIDefineByPos()) use the external
   type code SQLT_NTY.
 */

/* OCIColl_ - generic collection type */
struct OCIColl_;

/* OCIArray - varray collection type */
alias OCIArray = OCIColl_;

/* OCITable - nested table collection type */
alias OCITable = OCIColl_;
...
sword OCICollSize (OCIEnv* env, OCIError* err, const(OCIColl_)* coll, sb4* size);
/*
   NAME: OCICollSize - OCIColl_ return current SIZe of the given collection
   PARAMETERS:
        env(IN) - pointer to OCI environment handle
        err (IN/OUT) - error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
...
sb4 OCICollMax (OCIEnv* env, const(OCIColl_)* coll);
/*
   NAME: OCICollMax - OCIColl_ return MAXimum size (upper-bound) of the
...
sword OCICollGetElem (
    ...
    void** elemind);
/*
   NAME: OCICollGetElem - OCIColl_ GET pointer to the element at the given index
   PARAMETERS:
        env (IN/OUT) - OCI environment handle initialized in object mode.
        err (IN/OUT) - error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
   ...
   DESCRIPTION:
        ...
        -----------------------               ---------------
         ...
         object reference (OCIRef_*)             OCIRef_**
        ...
        a collection whose element type is object reference (OCIRef_*). A call
        to OCICollGetElem() returns pointer to a reference handle
        (i.e. OCIRef_**). After getting, the pointer to the collection
        ...
        sword OCIRefAssign( OCIEnv *env, OCIError *err, const OCIRef_ *source,
                            OCIRef_ **target );

        Note that the 'target' parameter of OCIRefAssign() is of type
        'OCIRef_**'. Hence OCICollGetElem() returns 'OCIRef_**'.
...
sword OCICollGetElemArray (
    ...
    uword* nelems);
/*
   NAME: OCICollGetElemArray - OCIColl_ GET pointers to elements from given index
   PARAMETERS:
        env (IN/OUT) - OCI environment handle initialized in object mode.
        err (IN/OUT) - error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
...
sword OCICollAssignElem (
    ...
    OCIColl_* coll);
/*
   NAME: OCICollAssignElem - OCIColl_ ASsign Element
   PARAMETERS:
        env (IN/OUT) - OCI environment handle initialized in object mode.
        err (IN/OUT) - error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
...
sword OCICollAssign (
    ...
    OCIColl_* lhs);
/*
   NAME: OCICollAssign - OCIColl_ ASsiGn collection
   PARAMETERS:
        env (IN/OUT) - OCI environment handle initialized in object mode.
        err (IN/OUT) - error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
...
sword OCICollAppend (
    ...
    OCIColl_* coll);
/*
   NAME: OCICollAppend - OCIColl_ APPend collection
   PARAMETERS:
        env (IN/OUT) - OCI environment handle initialized in object mode.
        err (IN/OUT) - error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
...
sword OCICollTrim (OCIEnv* env, OCIError* err, sb4 trim_num, OCIColl_* coll);
/*
   NAME: OCICollTrim - OCIColl_ Trim elements from the end of the collection
   PARAMETERS:
        env (IN/OUT) - OCI environment handle initialized in object mode.
        err (IN/OUT) - error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
...
sword OCICollIsLocator (
    OCIEnv* env,
    OCIError* err,
    const(OCIColl_)* coll,
    int* result);
/*
Name: OCICollIsLocator - OCIColl_ indicates whether a collection is locator
                         based or not.
Parameters:
        env(IN) - pointer to OCI environment handle
        err (IN/OUT) - error handle. If there is an error, it is
...
sword OCIIterCreate (
    ...
    OCIIter** itr);
/*
   NAME: OCIIterCreate - OCIColl_ Create an ITerator to scan the collection
                      elements
   PARAMETERS:
        env (IN/OUT) - OCI environment handle initialized in object mode.
        err (IN/OUT) - error handle. If there is an error, it is
...
/*
   NAME: OCIIterDelete - OCIColl_ Delete ITerator
   PARAMETERS:
        env (IN/OUT) - OCI environment handle initialized in object mode.
        err (IN/OUT) - error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
...
sword OCIIterInit (
    ...
    OCIIter* itr);
/*
   NAME: OCIIterInit - OCIColl_ Initialize ITerator to scan the given
                   collection
   PARAMETERS:
        env (IN/OUT) - OCI environment handle initialized in object mode.
        err (IN/OUT) - error handle. If there is an error, it is
...
sword OCIIterGetCurrent (
    ...
    void** elemind);
/*
   NAME: OCIIterGetCurrent - OCIColl_ Iterator based, get CURrent collection
                    element
   PARAMETERS:
        env (IN/OUT) - OCI environment handle initialized in object mode.
        err (IN/OUT) - error handle. If there is an error, it is
...
sword OCIIterNext (
    ...
    int* eoc);
/*
   NAME: OCIIterNext - OCIColl_ Iterator based, get NeXT collection element
   PARAMETERS:
        env (IN/OUT) - OCI environment handle initialized in object mode.
        err (IN/OUT) - error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
...
sword OCIIterPrev (
    ...
    int* boc);
/*
   NAME: OCIIterPrev - OCIColl_ Iterator based, get PReVious collection element
   PARAMETERS:
        env (IN/OUT) - OCI environment handle initialized in object mode.
        err (IN/OUT) - error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
