...
/*
  ...
  MODIFIED
    ...
    lchidamb   01/20/97 - update OCIRef_ comments
...
enum OCI_IND_NOTNULL = cast(OCIInd) 0; /* not NULL */
enum OCI_IND_NULL = cast(OCIInd) -1; /* NULL */
enum OCI_IND_BADNULL = cast(OCIInd) -2; /* BAD NULL */
enum OCI_IND_NOTNULLABLE = cast(OCIInd) -3; /* not NULLable */
...
enum OCI_ATTR_CACHE_ARRAYFLUSH = 0x00000040;
...
alias OCIDuration = ushort;

enum OCI_DURATION_INVALID = 0xFFFF; /* Invalid duration */
enum OCI_DURATION_BEGIN = cast(OCIDuration) 10;
/* beginning sequence of duration */
enum OCI_DURATION_NULL = cast(OCIDuration) OCI_DURATION_BEGIN - 1;
/* null duration */
enum OCI_DURATION_DEFAULT = cast(OCIDuration) OCI_DURATION_BEGIN - 2; /* default */
enum OCI_DURATION_USER_CALLBACK = cast(OCIDuration) OCI_DURATION_BEGIN - 3;
enum OCI_DURATION_NEXT = cast(OCIDuration) OCI_DURATION_BEGIN - 4;
/* next special duration */
enum OCI_DURATION_SESSION = cast(OCIDuration) OCI_DURATION_BEGIN;
/* the end of user session */
enum OCI_DURATION_TRANS = cast(OCIDuration) OCI_DURATION_BEGIN + 1;
/* the end of user transaction */
/******************************************************************************
**  DO NOT USE OCI_DURATION_CALL. IT  IS UNSUPPORTED                         **
**  WILL BE REMOVED/CHANGED IN A FUTURE RELEASE                              **
******************************************************************************/
enum OCI_DURATION_CALL = cast(OCIDuration) OCI_DURATION_BEGIN + 2;
/* the end of user client/server call */
enum OCI_DURATION_STATEMENT = cast(OCIDuration) OCI_DURATION_BEGIN + 3;
...
enum OCI_DURATION_CALLOUT = cast(OCIDuration) OCI_DURATION_BEGIN + 4;

enum OCI_DURATION_LAST = OCI_DURATION_CALLOUT;
...
*/
enum OCI_DURATION_PROCESS = cast(OCIDuration) OCI_DURATION_BEGIN - 5;
...
enum OCIObjectProperty
{
    ...
    OCI_OBJECTPROP_LOADED = 2, /* objects loaded in the transaction */
    OCI_OBJECTPROP_LOCKED = 3 /* locked objects */
}
...
enum OCIObjectEvent
{
    ...
    OCI_OBJECTEVENT_AFTER_FLUSH = 2, /* after flushing the cache */
    OCI_OBJECTEVENT_BEFORE_REFRESH = 3, /* before refreshing the cache */
    OCI_OBJECTEVENT_AFTER_REFRESH = 4, /* after refreshing the cache */
    OCI_OBJECTEVENT_WHEN_MARK_UPDATED = 5, /* when an object is marked updated */
    OCI_OBJECTEVENT_WHEN_MARK_DELETED = 6, /* when an object is marked deleted */
    OCI_OBJECTEVENT_WHEN_UNMARK = 7, /* when an object is being unmarked */
    OCI_OBJECTEVENT_WHEN_LOCK = 8 /* when an object is being locked */
}
...
/*----------------------------- OBJECT COPY OPTION --------------------------*/
enum OCI_OBJECTCOPY_NOREF = cast(ub1) 0x01;
/*
...
/*----------------------------- OBJECT FREE OPTION --------------------------*/
enum OCI_OBJECTFREE_FORCE = cast(ub2) 0x0001;
enum OCI_OBJECTFREE_NONULL = cast(ub2) 0x0002;
enum OCI_OBJECTFREE_HEADER = cast(ub2) 0x0004;
/*
...
alias OCIObjectPropId = ubyte;
enum OCI_OBJECTPROP_LIFETIME = 1; /* persistent or transient or value */
enum OCI_OBJECTPROP_SCHEMA = 2; /* schema name of table containing object */
enum OCI_OBJECTPROP_TABLE = 3; /* table name of table containing object */
enum OCI_OBJECTPROP_PIN_DURATION = 4; /* pin duartion of object */
enum OCI_OBJECTPROP_ALLOC_DURATION = 5; /* alloc duartion of object */
enum OCI_OBJECTPROP_LOCK = 6; /* lock status of object */
enum OCI_OBJECTPROP_MARKSTATUS = 7; /* mark status of object */
enum OCI_OBJECTPROP_VIEW = 8; /* is object a view object or not? */
...
alias OCIObjectMarkStatus = uint;
enum OCI_OBJECT_NEW = 0x0001; /* new object */
enum OCI_OBJECT_DELETED = 0x0002; /* object marked deleted */
enum OCI_OBJECT_UPDATED = 0x0004; /* object marked updated */
/*
...
extern (D) auto OCI_OBJECT_IS_UPDATED(T)(auto ref T flag)
{
    return bit(flag, OCI_OBJECT_UPDATED);
}

extern (D) auto OCI_OBJECT_IS_DELETED(T)(auto ref T flag)
{
    return bit(flag, OCI_OBJECT_DELETED);
}

extern (D) auto OCI_OBJECT_IS_NEW(T)(auto ref T flag)
{
    return bit(flag, OCI_OBJECT_NEW);
}
...
enum OCI_TYPECODE_REF = SQLT_REF; /* SQL/OTS OBJECT REFERENCE */
enum OCI_TYPECODE_DATE = SQLT_DAT; /* SQL DATE  OTS DATE */
enum OCI_TYPECODE_SIGNED8 = 27; /* SQL SIGNED INTEGER(8)  OTS SINT8 */
enum OCI_TYPECODE_SIGNED16 = 28; /* SQL SIGNED INTEGER(16)  OTS SINT16 */
enum OCI_TYPECODE_SIGNED32 = 29; /* SQL SIGNED INTEGER(32)  OTS SINT32 */
enum OCI_TYPECODE_REAL = 21; /* SQL REAL  OTS SQL_REAL */
enum OCI_TYPECODE_DOUBLE = 22; /* SQL DOUBLE PRECISION  OTS SQL_DOUBLE */
enum OCI_TYPECODE_BFLOAT = SQLT_IBFLOAT; /* Binary float */
enum OCI_TYPECODE_BDOUBLE = SQLT_IBDOUBLE; /* Binary double */
enum OCI_TYPECODE_FLOAT = SQLT_FLT; /* SQL FLOAT(P)  OTS FLOAT(P) */
enum OCI_TYPECODE_NUMBER = SQLT_NUM; /* SQL NUMBER(P S)  OTS NUMBER(P S) */
enum OCI_TYPECODE_DECIMAL = SQLT_PDN;
/* SQL DECIMAL(P S)  OTS DECIMAL(P S) */
enum OCI_TYPECODE_UNSIGNED8 = SQLT_BIN;
/* SQL UNSIGNED INTEGER(8)  OTS UINT8 */
enum OCI_TYPECODE_UNSIGNED16 = 25; /* SQL UNSIGNED INTEGER(16)  OTS UINT16 */
enum OCI_TYPECODE_UNSIGNED32 = 26; /* SQL UNSIGNED INTEGER(32)  OTS UINT32 */
enum OCI_TYPECODE_OCTET = 245; /* SQL ???  OTS OCTET */
enum OCI_TYPECODE_SMALLINT = 246; /* SQL SMALLINT  OTS SMALLINT */
enum OCI_TYPECODE_INTEGER = SQLT_INT; /* SQL INTEGER  OTS INTEGER */
enum OCI_TYPECODE_RAW = SQLT_LVB; /* SQL RAW(N)  OTS RAW(N) */
enum OCI_TYPECODE_PTR = 32; /* SQL POINTER  OTS POINTER */
enum OCI_TYPECODE_VARCHAR2 = SQLT_VCS;
/* SQL VARCHAR2(N)  OTS SQL_VARCHAR2(N) */
enum OCI_TYPECODE_CHAR = SQLT_AFC; /* SQL CHAR(N)  OTS SQL_CHAR(N) */
enum OCI_TYPECODE_VARCHAR = SQLT_CHR;
/* SQL VARCHAR(N)  OTS SQL_VARCHAR(N) */
enum OCI_TYPECODE_MLSLABEL = SQLT_LAB; /* OTS MLSLABEL */
enum OCI_TYPECODE_VARRAY = 247; /* SQL VARRAY  OTS PAGED VARRAY */
enum OCI_TYPECODE_TABLE = 248; /* SQL TABLE  OTS MULTISET */
enum OCI_TYPECODE_OBJECT = SQLT_NTY; /* SQL/OTS NAMED OBJECT TYPE */
enum OCI_TYPECODE_OPAQUE = 58; /*  SQL/OTS Opaque Types */
enum OCI_TYPECODE_NAMEDCOLLECTION = SQLT_NCO;
/* SQL/OTS NAMED COLLECTION TYPE */
enum OCI_TYPECODE_BLOB = SQLT_BLOB; /* SQL/OTS BINARY LARGE OBJECT */
enum OCI_TYPECODE_BFILE = SQLT_BFILE; /* SQL/OTS BINARY FILE OBJECT */
enum OCI_TYPECODE_CLOB = SQLT_CLOB; /* SQL/OTS CHARACTER LARGE OBJECT */
enum OCI_TYPECODE_CFILE = SQLT_CFILE; /* SQL/OTS CHARACTER FILE OBJECT */

/* the following are ANSI datetime datatypes added in 8.1 */
enum OCI_TYPECODE_TIME = SQLT_TIME; /* SQL/OTS TIME */
enum OCI_TYPECODE_TIME_TZ = SQLT_TIME_TZ; /* SQL/OTS TIME_TZ */
enum OCI_TYPECODE_TIMESTAMP = SQLT_TIMESTAMP; /* SQL/OTS TIMESTAMP */
enum OCI_TYPECODE_TIMESTAMP_TZ = SQLT_TIMESTAMP_TZ; /* SQL/OTS TIMESTAMP_TZ */

enum OCI_TYPECODE_TIMESTAMP_LTZ = SQLT_TIMESTAMP_LTZ; /* TIMESTAMP_LTZ */

enum OCI_TYPECODE_INTERVAL_YM = SQLT_INTERVAL_YM; /* SQL/OTS INTRVL YR-MON */
enum OCI_TYPECODE_INTERVAL_DS = SQLT_INTERVAL_DS; /* SQL/OTS INTRVL DAY-SEC */
enum OCI_TYPECODE_UROWID = SQLT_RDD; /* Urowid type */

enum OCI_TYPECODE_OTMFIRST = 228; /* first Open Type Manager typecode */
enum OCI_TYPECODE_OTMLAST = 320; /* last OTM typecode */
enum OCI_TYPECODE_SYSFIRST = 228; /* first OTM system type (internal) */
enum OCI_TYPECODE_SYSLAST = 235; /* last OTM system type (internal) */
enum OCI_TYPECODE_PLS_INTEGER = 266; /* type code for PLS_INTEGER */

// Removed by Adam Walker 18-Jul-2021.  I have no idea how to resolve these.
///* the following are PL/SQL-only internal. They should not be used */
//enum OCI_TYPECODE_ITABLE = SQLT_TAB; /* PLSQL indexed table */
//enum OCI_TYPECODE_RECORD = SQLT_REC; /* PLSQL record */
//enum OCI_TYPECODE_BOOLEAN = SQLT_BOL; /* PLSQL boolean */
...
{
    /* 0 = uninitialized */
    OCI_TYPEENCAP_PRIVATE = 0, /* private: only internally visible */
    OCI_TYPEENCAP_PUBLIC = 1 /* public: visible to both internally and externally */
}
...
extern (D) auto OCI_METHOD_IS_INLINE(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_INLINE);
}

extern (D) auto OCI_METHOD_IS_CONSTANT(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_CONSTANT);
}

extern (D) auto OCI_METHOD_IS_VIRTUAL(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_VIRTUAL);
}

extern (D) auto OCI_METHOD_IS_CONSTRUCTOR(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_CONSTRUCTOR);
}

extern (D) auto OCI_METHOD_IS_DESTRUCTOR(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_DESTRUCTOR);
}

extern (D) auto OCI_METHOD_IS_OPERATOR(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_OPERATOR);
}

extern (D) auto OCI_METHOD_IS_SELFISH(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_SELFISH);
}

extern (D) auto OCI_METHOD_IS_MAP(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_MAP);
}

extern (D) auto OCI_METHOD_IS_ORDER(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_ORDER);
}

extern (D) auto OCI_METHOD_IS_RNDS(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_RNDS);
}

extern (D) auto OCI_METHOD_IS_WNDS(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_WNDS);
}

extern (D) auto OCI_METHOD_IS_RNPS(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_RNPS);
}

extern (D) auto OCI_METHOD_IS_WNPS(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_WNPS);
}

extern (D) auto OCI_METHOD_IS_ABSTRACT(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_ABSTRACT);
}

extern (D) auto OCI_METHOD_IS_OVERRIDING(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_OVERRIDING);
}

extern (D) auto OCI_METHOD_IS_PIPELINED(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_PIPELINED);
}

extern (D) auto OCI_TYPEMETHOD_IS_INLINE(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_INLINE);
}

extern (D) auto OCI_TYPEMETHOD_IS_CONSTANT(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_CONSTANT);
}

extern (D) auto OCI_TYPEMETHOD_IS_VIRTUAL(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_VIRTUAL);
}

extern (D) auto OCI_TYPEMETHOD_IS_CONSTRUCTOR(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_CONSTRUCTOR);
}

extern (D) auto OCI_TYPEMETHOD_IS_DESTRUCTOR(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_DESTRUCTOR);
}

extern (D) auto OCI_TYPEMETHOD_IS_OPERATOR(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_OPERATOR);
}

extern (D) auto OCI_TYPEMETHOD_IS_SELFISH(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_SELFISH);
}

extern (D) auto OCI_TYPEMETHOD_IS_MAP(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_MAP);
}

extern (D) auto OCI_TYPEMETHOD_IS_ORDER(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_ORDER);
}

extern (D) auto OCI_TYPEMETHOD_IS_RNDS(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_RNDS);
}

extern (D) auto OCI_TYPEMETHOD_IS_WNDS(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_WNDS);
}

extern (D) auto OCI_TYPEMETHOD_IS_RNPS(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_RNPS);
}

extern (D) auto OCI_TYPEMETHOD_IS_WNPS(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_WNPS);
}

extern (D) auto OCI_TYPEMETHOD_IS_ABSTRACT(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_ABSTRACT);
}

extern (D) auto OCI_TYPEMETHOD_IS_OVERRIDING(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_OVERRIDING);
}

extern (D) auto OCI_TYPEMETHOD_IS_PIPELINED(T)(auto ref T flag)
{
    return bit(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_PIPELINED);
}

/* macros to set the type method flags */
extern (D) auto OCI_TYPEMETHOD_SET_INLINE(T)(auto ref T flag)
{
    return bis(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_INLINE);
}

extern (D) auto OCI_TYPEMETHOD_SET_CONSTANT(T)(auto ref T flag)
{
    return bis(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_CONSTANT);
}

extern (D) auto OCI_TYPEMETHOD_SET_VIRTUAL(T)(auto ref T flag)
{
    return bis(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_VIRTUAL);
}

extern (D) auto OCI_TYPEMETHOD_SET_CONSTRUCTOR(T)(auto ref T flag)
{
    return bis(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_CONSTRUCTOR);
}

extern (D) auto OCI_TYPEMETHOD_SET_DESTRUCTOR(T)(auto ref T flag)
{
    return bis(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_DESTRUCTOR);
}

extern (D) auto OCI_TYPEMETHOD_SET_OPERATOR(T)(auto ref T flag)
{
    return bis(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_OPERATOR);
}

extern (D) auto OCI_TYPEMETHOD_SET_SELFISH(T)(auto ref T flag)
{
    return bis(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_SELFISH);
}

extern (D) auto OCI_TYPEMETHOD_SET_MAP(T)(auto ref T flag)
{
    return bis(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_MAP);
}

extern (D) auto OCI_TYPEMETHOD_SET_ORDER(T)(auto ref T flag)
{
    return bis(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_ORDER);
}

extern (D) auto OCI_TYPEMETHOD_SET_RNDS(T)(auto ref T flag)
{
    return bis(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_RNDS);
}

extern (D) auto OCI_TYPEMETHOD_SET_WNDS(T)(auto ref T flag)
{
    return bis(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_WNDS);
}

extern (D) auto OCI_TYPEMETHOD_SET_RNPS(T)(auto ref T flag)
{
    return bis(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_RNPS);
}

extern (D) auto OCI_TYPEMETHOD_SET_WNPS(T)(auto ref T flag)
{
    return bis(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_WNPS);
}

/* macros to clear the type method flags */
extern (D) auto OCI_TYPEMETHOD_CLEAR_INLINE(T)(auto ref T flag)
{
    return bic(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_INLINE);
}

extern (D) auto OCI_TYPEMETHOD_CLEAR_CONSTANT(T)(auto ref T flag)
{
    return bic(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_CONSTANT);
}

extern (D) auto OCI_TYPEMETHOD_CLEAR_VIRTUAL(T)(auto ref T flag)
{
    return bic(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_VIRTUAL);
}

extern (D) auto OCI_TYPEMETHOD_CLEAR_CONSTRUCTOR(T)(auto ref T flag)
{
    return bic(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_CONSTRUCTOR);
}

extern (D) auto OCI_TYPEMETHOD_CLEAR_DESTRUCTOR(T)(auto ref T flag)
{
    return bic(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_DESTRUCTOR);
}

extern (D) auto OCI_TYPEMETHOD_CLEAR_OPERATOR(T)(auto ref T flag)
{
    return bic(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_OPERATOR);
}

extern (D) auto OCI_TYPEMETHOD_CLEAR_SELFISH(T)(auto ref T flag)
{
    return bic(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_SELFISH);
}

extern (D) auto OCI_TYPEMETHOD_CLEAR_MAP(T)(auto ref T flag)
{
    return bic(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_MAP);
}

extern (D) auto OCI_TYPEMETHOD_CLEAR_ORDER(T)(auto ref T flag)
{
    return bic(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_ORDER);
}

extern (D) auto OCI_TYPEMETHOD_CLEAR_RNDS(T)(auto ref T flag)
{
    return bic(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_RNDS);
}

extern (D) auto OCI_TYPEMETHOD_CLEAR_WNDS(T)(auto ref T flag)
{
    return bic(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_WNDS);
}

extern (D) auto OCI_TYPEMETHOD_CLEAR_RNPS(T)(auto ref T flag)
{
    return bic(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_RNPS);
}

extern (D) auto OCI_TYPEMETHOD_CLEAR_WNPS(T)(auto ref T flag)
{
    return bic(flag, OCITypeMethodFlag.OCI_TYPEMETHOD_WNPS);
}
...
enum OCITypeParamMode
{
    ...
    OCI_TYPEPARAM_OUTNCPY = 4, /* OUT with NOCOPY modifier */
    OCI_TYPEPARAM_INOUTNCPY = 5 /* IN OUT with NOCOPY modifier */
}
...
enum OCI_NUMBER_DEFAULTPREC = cast(ub1) 0; /* no precision specified */
enum OCI_NUMBER_DEFAULTSCALE = cast(sb1) MAXSB1MINVAL;
/* no binary/decimal scale specified */

/* default maximum length for varrays and vstrings (used in sql.bsq) */

enum OCI_VARRAY_MAXSIZE = 4000;
/* default maximum number of elements for a varray */
enum OCI_STRING_MAXLEN = 4000; /* default maximum length of a vstring */
 ...
 */
alias OCICoherency = OCIRefreshOpt;
enum OCI_COHERENCY_NONE = cast(OCIRefreshOpt) 2;
enum OCI_COHERENCY_NULL = cast(OCIRefreshOpt) 4;
enum OCI_COHERENCY_ALWAYS = cast(OCIRefreshOpt) 5;
