...
/*
  ...
  DESCRIPTION
    ...
    NOTE: MOST Of the functions in this header file are being desupported.
          ...
          instead.
          The OCIType_, OCITypeElem, OCITypeMethod abstract types continue
  ...
  **** ALL OBSOLETE FUNCTIONS/TYPES ARE MARKED ACCORDINGLY ***
  ...
  PUBLIC DATA STRUCTURES
      ...
      OCIType_ - type descriptor in the object cache
  ...
  ** OBSOLETE ** please use OCIDescribe interface
  ...
  TYPE ACCESSOR EXAMPLE
      ...
      OCIRef_  *attr_ref;
      OCIRef_  *param_ref;
      OCIType_  *tdo, new_tdo, final_tdo;
      OCITypeElem  *elem;
      ...
      if (OCITypeMethodByName(env, err, car_tdo, "car_constructor",
                              strlen("car_constructor"), NULLP(OCIRef_), &mdo)
                              != OCI_SUCCESS)
        /o error o/ ;
      ...
      if (ortgpps(env, err, mdo, "an_age", strlen("an_age"),
                  &position, NULLP(OCIRef_), NULLP(OCITypeElem)) != OCI_SUCCESS)
        /o error o/ ;
  ...
  MODIFIED
    ...
    rxgovind   01/31/00  - add OCIType_ interfaces for transient types
    whe        09/01/99 -  976457:check __cplusplus for C++ code
    cxcheng    05/06/97 -  make OCI_TYPE?? test macros return either 1 or 0
    cxcheng    04/22/97 -  add comment on desupporting OCIType_ functions
    ...
    cxcheng    12/09/96 -  add prototype for OCITypeElemExtTypeCode and OCIType_
...
/*
 * OCIType_ - OCI Type Description Object
 *
 * The contents of an 'OCIType_' is private/opaque to clients.  Clients just
 * need to declare and pass 'OCIType_' pointers in to the type manage
 * functions.
...
/*
  ...
  PARAMETERS:
       ...
       err (IN/OUT) - error handle. If there is an error, it is
                ...
                The error recorded in 'err' can be retrieved by calling
                OCIErrorGet().
       type_ref (IN) - OCIRef_ * pointing to the particular version of
...
/*
  ...
  PARAMETERS:
       ...
       array_len (IN) - number of schema_name/type_name/version_name entries to
                  be retrieved.
       type_ref (IN) - array of OCIRef_ * pointing to the particular version of
                  the type descriptor object to obtain.
                  The array must have array_len elements if specified.
       pin_duration (IN) - pin duration (e.g. until the end of current
                  transaction) for the types retreieve.  See 'oro.h' for a
...
/* ** OBSOLETE ** */
OCITypeCode OCITypeTypeCode (OCIEnv* env, OCIError* err, const(OCIType_)* tdo);
/*
...
/* ** OBSOLETE ** */
OCITypeCode OCITypeCollTypeCode (
    OCIEnv* env,
    OCIError* err,
    const(OCIType_)* tdo);
/*
...
/* ** OBSOLETE ** */
ub4 OCITypeAttrs (OCIEnv* env, OCIError* err, const(OCIType_)* tdo);
/*
...
/* ** OBSOLETE ** */
ub4 OCITypeMethods (OCIEnv* env, OCIError* err, const(OCIType_)* tdo);
/*
...
/* ** OBSOLETE ** */
OCITypeCode OCITypeElemTypeCode (
    OCIEnv* env,
    OCIError* err,
    const(OCITypeElem)* elem);
/*
...
/* ** OBSOLETE ** */
sword OCITypeElemParameterizedType (
    OCIEnv* env,
    OCIError* err,
    const(OCITypeElem)* elem,
    OCIType_** type_stored);
/*
   ...
   PARAMETERS:
        ...
        type_stored (OUT) - If the function completes successfully,
               ...
               type.  The caller must allocate space for the OCIType_*
...
/* ** OBSOLETE ** */
OCITypeCode OCITypeElemExtTypeCode (
    OCIEnv* env,
    OCIError* err,
    const(OCITypeElem)* elem);
/*
...
/* ** OBSOLETE ** */
OCITypeEncap OCITypeMethodEncap (
    OCIEnv* env,
    OCIError* err,
    const(OCITypeMethod)* mdo);
/*
...
/* ** OBSOLETE ** */
OCITypeMethodFlag OCITypeMethodFlags (
    OCIEnv* env,
    OCIError* err,
    const(OCITypeMethod)* mdo);
/*
...
/* ** OBSOLETE ** */
OCITypeParamMode OCITypeElemParamMode (
    OCIEnv* env,
    OCIError* err,
    const(OCITypeElem)* elem);
/*
...
/* ** OBSOLETE ** */
oratext* OCITypeElemDefaultValue (
    OCIEnv* env,
    OCIError* err,
    const(OCITypeElem)* elem,
    ub4* d_v_length);
...
sword ortgcty (
    ...
    OCIError* err,
    OCIType_* coll_tdo,
    OCIType_** collelem_tdo);

/*---------------------------------------------------------------------------*/
/*               Transient Type Construction functions                       */
/*---------------------------------------------------------------------------*/

sword OCITypeBeginCreate (
    OCISvcCtx* svchp,
    OCIError* errhp,
    OCITypeCode tc,
    OCIDuration dur,
...
/*
   ...
   PARAMETERS:
       ...
       type(OUT)        - The OCIType_ (Type Descriptor) that is being
                          constructed.
  RETURNS:
        OCI_SUCCESS if the function completes successfully.
        OCI_ERROR on error.
...
/*
   ...
   REMARKS :
       ...
       OCIType_ has been constructed with a collection typecode.
   PARAMETERS:
       svchp (IN)      -  The OCI Service Context.
       errhp (IN/OUT)  -  The OCI error handle. If there is an error, it is
                          recorded in errhp and this function returns
                          OCI_ERROR. Diagnostic information can be obtained by
                          calling OCIErrorGet().
       type(IN OUT)    -  The OCIType_ (Type Descriptor) that is being
                          constructed.
...
/*
   ...
   REMARKS:
       ...
       OCIType_ has been constructed with a built-in typecode
...
sword OCITypeEndCreate (OCISvcCtx* svchp, OCIError* errhp, OCIType_* type);
/*
...
enum OCI_TYPEELEM_REF = 0x8000; /* element is a REF */
enum OCI_TYPEPARAM_REQUIRED = 0x0800; /* parameter is required */

/* macros to test flags */
extern (D) auto OCI_TYPEELEM_IS_REF(T)(auto ref T elem_flag)
{
    return (elem_flag & OCI_TYPEELEM_REF) != 0;
}

extern (D) auto OCI_TYPEPARAM_IS_REQUIRED(T)(auto ref T param_flag)
{
