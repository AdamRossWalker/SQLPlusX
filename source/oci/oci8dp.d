...
struct OCIDirPathCtx; /* context */
struct OCIDirPathFuncCtx; /* function context */
struct OCIDirPathColArray; /* column array */
struct OCIDirPathStream; /* stream */
struct OCIDirPathDesc; /* direct path descriptor */

/*----- Defines for Direct Path Options -----*/

/* values for OCI_ATTR_DIRPATH_MODE attribute */
enum OCI_DIRPATH_LOAD = 1; /* direct path load operation */
enum OCI_DIRPATH_UNLOAD = 2; /* direct path unload operation */
enum OCI_DIRPATH_CONVERT = 3; /* direct path convert only operation */

/*----- values for OCI_ATTR_DIRPATH_INDEX_MAINT_METHOD attribute -----*/
enum OCI_DIRPATH_INDEX_MAINT_SINGLE_ROW = 1;
...
enum OCI_DIRPATH_INDEX_MAINT_SKIP_UNUSABLE = 2;
enum OCI_DIRPATH_INDEX_MAINT_DONT_SKIP_UNUSABLE = 3;
enum OCI_DIRPATH_INDEX_MAINT_SKIP_ALL = 4;

/* values for OCI_ATTR_STATE attribute of OCIDirPathCtx */
enum OCI_DIRPATH_NORMAL = 1; /* can accept rows, last row complete */
enum OCI_DIRPATH_PARTIAL = 2; /* last row was partial */
enum OCI_DIRPATH_NOT_PREPARED = 3; /* direct path context is not prepared */

/*----- values for cflg argument to OCIDirpathColArrayEntrySet -----*/
enum OCI_DIRPATH_COL_COMPLETE = 0; /* column data is complete */
enum OCI_DIRPATH_COL_NULL = 1; /* column is null */
enum OCI_DIRPATH_COL_PARTIAL = 2; /* column data is partial */
enum OCI_DIRPATH_COL_ERROR = 3; /* column error, ignore row */
/*----- values for action parameter to OCIDirPathDataSave -----*/
enum OCI_DIRPATH_DATASAVE_SAVEONLY = 0; /* data save point only */
enum OCI_DIRPATH_DATASAVE_FINISH = 1; /* execute finishing logic */
/* save portion of input data (before space error occurred) and finish */
enum OCI_DIRPATH_DATASAVE_PARTIAL = 2;

/*- OCI_ATTR_DIRPATH_EXPR_TYPE values (describes OCI_ATTR_NAME expr type) -*/
enum OCI_DIRPATH_EXPR_OBJ_CONSTR = 1; /* NAME is an object constructor */
enum OCI_DIRPATH_EXPR_SQL = 2; /* NAME is an opaque or sql function */
enum OCI_DIRPATH_EXPR_REF_TBLNAME = 3; /* NAME is table name if ref is scoped*/
...
/*
  ...
  NOTES
 */
sword OCIDirPathColArrayEntryGet (
...
/*
  ...
  NOTES
 */
sword OCIDirPathColArrayEntrySet (
    OCIDirPathColArray* dpca,
    OCIError* errhp,
    ub4 rownum,
    ub2 colIdx,
...
/*
  ...
  NOTES
 */
sword OCIDirPathColArrayRowGet (
    OCIDirPathColArray* dpca,
    OCIError* errhp,
    ub4 rownum,
    ub1*** cvalppp,
...
/*
  ...
  NOTES
 */
sword OCIDirPathColArrayToStream (
    OCIDirPathColArray* dpca,
    OCIDirPathCtx* dpctx,
    OCIDirPathStream* dpstr,
    OCIError* errhp,
